        ...
        <view class="panel" :class="{ 'panel--show': visible }" @click.stop>
            <scroll-view class="panel__scroll" scroll-y>
                <view class="grid" :style="{ gridTemplateColumns: `repeat(${columns}, 1fr)` }">
                    <view v-for="opt in options" :key="opt.value" class="pill" hover-class="pill--pressed"
                        @click="select(opt)">
                        <view class="pill__inner" :class="{ 'pill__inner--active': opt.value === localSelected }">
                            <view class="pill__text">{{ opt.label }}</view>
                        </view>
                    </view>
                </view>
            </scroll-view>
...
const select = (opt: SalaryFilterOption) => {
