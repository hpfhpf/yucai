                    ...
                    <view class="stepper__nodes">
                        <view v-for="(s, i) in steps" :key="s" class="step">
                            <view class="dot" :class="dotClass(i)">
                                <wd-icon v-if="i < currentStep" name="check" size="14rpx"
                ...
                <template v-if="currentStep === 1">
                    <view class="card">
                        ...
                        <view class="searchRow">
                            <wd-icon name="search-line" size="28rpx" color="rgba(0,0,0,0.28)" />
                        ...
                        <view v-if="companyResults.length" class="companyList">
                            <view v-for="c in companyResults" :key="c.id" class="companyItem"
                                ...
                                <view v-if="form.companyId === c.id" class="companyItem__check">
                                    <wd-icon name="check" size="20rpx" color="rgba(30, 91, 255, 0.98)" />
                                </view>
                            </view>
                        </view>
                        <!-- 未找到企业 -->
                        ...
                        <view v-if="selectedCompany" class="selectedCard">
                            <wd-icon name="building" size="28rpx" color="rgba(30, 91, 255, 0.92)" />
                            ...
                            <view class="selectedCard__clear" hover-class="selectedCard__clear--pressed"
                                @click="clearCompany">
                                <wd-icon name="close" size="20rpx" color="rgba(0,0,0,0.38)" />
                            </view>
                        </view>
                    </view>
                ...
                <template v-if="currentStep === 2">
                    <view class="doneCard">
                        <view class="doneCard__icon">
                            <wd-icon name="check-circle" size="80rpx" color="rgba(30, 91, 255, 0.92)" />
                        </view>
...
const handleNext = async () => {
    ...
    if (currentStep.value === 1) {
        ...
        try {
            ...
            if (isEditMode.value) {
                await apiUpdateRecruiterProfile(payload)
            } else {
                await apiRegisterRecruiter(payload)
            }
            currentStep.value = 2
        } catch (e: any) {
            toast.info(e?.message || '提交失败，请重试')
        } finally {
