<template>
    <view class="manualContainer">
            ...
            <view class="content" :style="{ paddingBottom: `${safeBottom + 160}px` }">
                <view class="card">
                    ...
                    <view class="formRow">
                        ...
                        <view class="formRow__field">
                            <wd-input v-model="form.addressDetail" compact custom-class="formInput"
                                placeholder="请输入" />
                        </view>
                    </view>
                    <view class="divider" />
                    ...
                    <view class="formRow">
                        ...
                        <view class="formRow__field formRow__field--inline">
                            <wd-input v-model="form.annualSalaryMinStr" type="number" compact
                                custom-class="formInput formInput--narrow" placeholder="下限" />
                            <text class="formRow__sep">-</text>
                            <wd-input v-model="form.annualSalaryMaxStr" type="number" compact
                                custom-class="formInput formInput--narrow" placeholder="上限" />
                            <text class="formRow__unit">万/年</text>
                        </view>
                    </view>
                    <view class="divider" />

                    <view class="formRow formRow--tap" hover-class="formRow--pressed"
                        @click="openSheet('education')"
                    >
        ...
        <view class="bottomBar" :style="{ paddingBottom: `${safeBottom}px` }">
            ...
            <wd-button type="primary" custom-class="barBtn barBtn--primary" :loading="publishing"
                @click="handlePublish"
            >
                {{ editId ? '保存职位' : '发布职位' }}
            </wd-button>
            ...
            <view class="sheetContent">
                ...
                <scroll-view class="sheetContent__list" scroll-y>
                    ...
                    >
                        ...
                        <FaIcon v-if="sheetValue === opt" name="check" :size="28" color="rgba(30, 91, 255, 0.92)" />
                    </view>
                </scroll-view>
            ...
            <view class="previewPanel">
                ...
                <view class="previewCard">
                    ...
                    <view class="previewCard__section">
                        ...
                        <view class="previewCard__text">
                            {{ [form.province, form.city, form.district, form.addressDetail].filter(Boolean).join(' ') || '未填写' }}
                        </view>
                    </view>
                    <view class="previewCard__divider" />
                    <view class="previewCard__section">
        ...
        <wd-toast selector="manualJobToast" />
    </view>
</template>
...
import { apiCreateJob, apiUpdateJob } from '@/api/index'

const props = defineProps<{
    initialData?: Record<string, any> | null
}>()

const toast = useToast('manualJobToast')
...
// 当父组件传入 AI 生成数据时，填充表单
watch(() => props.initialData, (data) => {
    if (!data) return
    form.value.title = data.title || ''
    form.value.nature = data.nature || 'part'
    ...
    form.value.annualSalaryMinStr = data.annualSalaryMinStr || ''
    form.value.annualSalaryMaxStr = data.annualSalaryMaxStr || ''
    form.value.education = data.education || ''
    form.value.experience = data.experience || ''
    form.value.description = data.description || ''
    form.value.tags = Array.isArray(data.tags) ? data.tags : ['五险一金']
}, { immediate: true })
...
const openSheet = (key: SheetKey) => {
    ...
    if (key === 'province') {
        ...
        if (form.value.salary && !opts.includes(form.value.salary)) opts.unshift(form.value.salary)
...
const closeSheet = () => { sheetShown.value = false }

const selectSheetValue = (v: string) => {
    sheetValue.value = v
    const key = sheetKey.value
    if (key === 'province') {
        if (form.value.province !== v) { form.value.province = v; form.value.city = ''; form.value.district = '' }
    } else if (key === 'city') {
        if (form.value.city !== v) { form.value.city = v; form.value.district = '' }
    } else if (key === 'district') { form.value.district = v
    } else if (key === 'salary') { form.value.salary = v
    } else if (key === 'education') { form.value.education = v
    } else if (key === 'experience') { form.value.experience = v }
    closeSheet()
}

const validateForm = () => {
    if (!form.value.title.trim()) return '请填写职位名称'
...
const natureEnumMap: Record<string, string> = { full: 'FULL_TIME', part: 'PART_TIME', intern: 'INTERNSHIP' }
const educationEnumMap: Record<string, string> = {
    '不限': 'ANY', '大专': 'ASSOCIATE', '本科': 'BACHELOR', '硕士': 'MASTER', '博士': 'DOCTOR',
}
const experienceYearsMap: Record<string, number> = {
    '不限': 0, '1年以内': 0, '1-3年': 1, '3-5年': 3, '5-10年': 5, '10年以上': 10,
}
...
const doPublish = async () => {
    const err = validateForm()
    if (err) { toast.info(err); return false }
...
const handlePublish = () => { void doPublish() }

// 供父组件调用：AI 生成后直接发布
const directPublish = () => { void doPublish() }

defineExpose({ directPublish })
</script>
...
.formRow--pressed { background: rgba(0, 0, 0, 0.02); }
...
.formRow__field--inline { gap: 10rpx; }

.formRow__sep { color: var(--app-text-secondary); font-size: 28rpx; flex-shrink: 0; }

.formRow__unit { color: var(--app-text-secondary); font-size: 24rpx; flex-shrink: 0; margin-left: 4rpx; }

:deep(.formInput--narrow) { width: 120rpx !important; flex: none !important; }
...
:deep(.formInput) {
    ...
    .wd-input__inner { height: 96rpx; font-size: 28rpx; color: var(--app-text-primary) !important; }
    .uni-input-placeholder { color: var(--app-text-muted) !important; font-size: 28rpx; }
}

.formRow__value { width: 100%; font-size: 28rpx; font-weight: 750; color: var(--app-text-primary); }
.formRow__value--ph { color: var(--app-text-muted); font-weight: 700; }

.divider { height: 1px; background: var(--app-line); margin-left: 22rpx; }
...
.addr__text { font-size: 24rpx; font-weight: 850; color: var(--app-text-secondary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; min-width: 0; flex: 1 1 auto; }
.addr__text--ph { color: var(--app-text-muted); font-weight: 700; }

.blockTitle { font-size: 28rpx; font-weight: 900; color: var(--app-text-secondary); margin-bottom: 14rpx; }

:deep(.descInput) {
    width: 100%;
    min-height: 160rpx;
    background: transparent !important;
    .wd-textarea__inner { min-height: 160rpx; font-size: 28rpx; color: var(--app-text-primary) !important; line-height: 1.65; }
    .uni-textarea-placeholder { color: var(--app-text-muted) !important; font-size: 28rpx; }
}

.tags { display: flex; flex-wrap: wrap; gap: 16rpx; padding-bottom: 2rpx; }
...
.natureGroup { display: flex; gap: 16rpx; }
...
:deep(.barBtn--primary) { box-shadow: 0 18rpx 50rpx rgba(30, 91, 255, 0.26) !important; }

.sheetContent { padding-bottom: env(safe-area-inset-bottom); }
.sheetContent__title { padding: 18rpx 22rpx 12rpx; text-align: center; font-size: 28rpx; font-weight: 900; color: var(--app-text-primary); }
.sheetContent__list { max-height: 520rpx; }
.sheetContent__item { height: 92rpx; padding: 0 22rpx; display: flex; align-items: center; justify-content: space-between; box-sizing: border-box; }
.sheetContent__item + .sheetContent__item { border-top: 1px solid var(--app-line); }
.sheetContent__item--pressed { background: rgba(0, 0, 0, 0.02); }
.sheetContent__text { font-size: 28rpx; font-weight: 800; color: var(--app-text-secondary); }
.sheetContent__footer { padding: 14rpx 22rpx 18rpx; box-sizing: border-box; }
...
.previewPanel { padding: 22rpx 26rpx 30rpx; box-sizing: border-box; }
.previewPanel__hd { display: flex; align-items: center; justify-content: space-between; margin-bottom: 18rpx; }
.previewPanel__title { font-size: 32rpx; font-weight: 900; color: var(--app-text-primary); }
.previewPanel__close { width: 52rpx; height: 52rpx; display: flex; align-items: center; justify-content: center; }

.previewCard { border-radius: var(--app-radius-lg); background: var(--app-surface); border: 1px solid rgba(255, 255, 255, 0.88); box-shadow: var(--app-shadow-card); padding: 22rpx; box-sizing: border-box; }
.previewCard__top { display: flex; align-items: flex-start; justify-content: space-between; gap: 12rpx; margin-bottom: 14rpx; }
.previewCard__name { font-size: 34rpx; font-weight: 900; color: var(--app-text-primary); }
.previewCard__salary { font-size: 30rpx; font-weight: 900; color: rgba(30, 91, 255, 0.98); flex: 0 0 auto; }
.previewCard__tags { display: flex; flex-wrap: wrap; gap: 12rpx; margin-bottom: 18rpx; }
.previewCard__divider { height: 1px; background: var(--app-line); margin: 14rpx 0; }
.previewCard__label { font-size: 24rpx; font-weight: 900; color: var(--app-text-muted); margin-bottom: 8rpx; }
.previewCard__text { font-size: 28rpx; color: var(--app-text-secondary); line-height: 1.6; }
.previewCard__perks { margin-top: 14rpx; display: flex; flex-wrap: wrap; gap: 12rpx; }
</style>
