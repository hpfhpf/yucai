<template>
    <view class="page">
                    ...
                    <view class="hero">
                        <view class="hero__left">
                            ...
                            <view class="hero__cityRow" v-if="profile.city">
                                <FaIcon name="location-dot" :size="20" color="rgba(0, 0, 0, 0.46)" />
        ...
        <view v-show="!showInvite" class="bottomBar" :style="{ paddingBottom: `${safeBottom}px` }">
            <view class="bottomBar__left">
                <view class="miniBtn" hover-class="miniBtn--pressed" @click="handleReject">
                    <FaIcon name="xmark" :size="28" color="rgba(0, 0, 0, 0.46)" />
                    <view class="miniBtn__text">不合适</view>
                </view>
                <view class="miniBtn" hover-class="miniBtn--pressed" @click="handleOpenChat">
                    <FaIcon name="comment-dots" :size="28" color="rgba(30, 91, 255, 0.76)" />
                    <view class="miniBtn__text">沟通</view>
                    <view v-if="chatUnread > 0" class="miniBtn__badge">{{ chatUnread > 99 ? '99+' : chatUnread }}</view>
                </view>
            </view>
        ...
        <!-- 聊天沟通弹窗 -->
        <ChatPopup v-model="showChat" :target-name="chatTargetName" :target-user-id="userId"
            @unread-count="onChatUnread" />
...
const chatTargetName = computed(() => {
    if (!profile.value) return '求职者'
    return profile.value.realName || profile.value.nickname || '求职者'
})
...
const onChatUnread = (count: number) => {
...
.miniBtn__badge {
    ...
    align-items: center;
    justify-content: center;
    box-shadow: 0 4rpx 12rpx rgba(255, 59, 48, 0.32);
}
