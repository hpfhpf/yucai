...
const minimatch__default = /*#__PURE__*/_interopDefaultCompat(minimatch);
...
function matchGlobs(filepath, globs) {
  for (const glob of globs) {
    if (minimatch__default(utils.slash(filepath), glob))
      return true;
  }
  return false;
}
...
function stringifyComponentImport({ as: name, from: path, name: importName, sideEffects }, ctx) {
  ...
  if (sideEffects)
    utils.toArray(sideEffects).forEach((i) => imports.push(stringifyImport(i)));
  return imports.join(";");
}
function getNameFromFilePath(filePath, options) {
  ...
  const parsedFilePath = node_path.parse(utils.slash(filePath));
  let strippedPath = "";
...
exports.DIRECTIVE_IMPORT_PREFIX = DIRECTIVE_IMPORT_PREFIX;
...
exports.normalizeComponetInfo = normalizeComponetInfo;
exports.parseId = parseId;
exports.pascalCase = pascalCase;
exports.resolveAlias = resolveAlias;
exports.shouldTransform = shouldTransform;
exports.stringifyComponentImport = stringifyComponentImport;
